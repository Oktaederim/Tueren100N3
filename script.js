...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const syncInputs = (slider, numberInput) => {
        // Vom Slider zum Zahlenfeld
        slider.addEventListener('input', () => {
            numberInput.value = parseFloat(slider.value).toFixed(2);
            updateCalculations();
        });
        // Vom Zahlenfeld zum Slider (live input)
        numberInput.addEventListener('input', () => {
            let value = parseFloat(numberInput.value);
            if (isNaN(value)) { // Wenn Eingabe ungültig (z.B. nur "-"), Slider nicht ändern
                // Optional: numberInput.value = ""; // Oder letzter gültiger Wert
                return; 
            }
        ...
        numberInput.addEventListener('change', () => {
            ...
            numberInput.value = value.toFixed(2); // Final formatierung nach Eingabe
            slider.value = value;
            updateCalculations();
        });
    };
